...
export async function buildSwapTransactions({
  ...
  if (!skipWithdraw && needsWithdraw) {
    ...
    if (withdrawResult && withdrawResult.obj) {
      ...
      // Only add asset opt-in fields if it's an ASA (use explicit type check)
      if (outputTokenType === 'ASA') {
        withdrawTxn.xaid = outputTokenIdNum;
        withdrawTxn.snd = acc.addr;
        withdrawTxn.arcv = acc.addr;
        // Add the underlying ASA to foreignAssets so the contract can do inner asset transfer
        withdrawTxn.foreignAssets = [outputTokenIdNum];
      }
  ...
  for (const txn of allTxns) {
    ...
    if (txn.appCallTxn) {
      ...
      // DEBUG: Log what foreignApps are coming from arccjs
      if (DEBUG) {
        console.log(`[HumbleSwap-arccjs] appCallTxn processing for appIndex=${txn.appCallTxn.applicationCall?.appIndex ?? txn.appCallTxn.appIndex}:`);
        console.log(`  txn.foreignApps:`, txn.foreignApps);
        console.log(`  txn.appCallTxn.applicationCall?.foreignApps:`, txn.appCallTxn.applicationCall?.foreignApps);
        console.log(`  txn.appCallTxn.foreignApps:`, txn.appCallTxn.foreignApps);
        console.log(`  finalForeignApps:`, finalForeignApps);
      }
      ...
      // DEBUG: Log what was actually set on applicationCall
      if (DEBUG) {
        console.log(`[HumbleSwap-arccjs] After setting on applicationCall:`);
        console.log(`  appCall.foreignApps:`, appCall.foreignApps);
        console.log(`  appCall.boxes:`, appCall.boxes?.map(b => ({ appIndex: Number(b.appIndex), nameHex: Buffer.from(b.name).toString('hex').substring(0, 20) })));
      }

      txns.push(appCallTxn);
      txnIndex++;
    } else if (txnAppIndex) {
      // DEBUG: Log what's coming from buildO for this transaction
      if (DEBUG) {
        console.log(`[HumbleSwap-arccjs] buildO txn processing for appIndex=${txnAppIndex}:`);
        console.log(`  txn.foreignApps:`, txn.foreignApps?.map(a => Number(a)));
      }
      ...
      // DEBUG: Log what was actually set on applicationCall
      if (DEBUG) {
        console.log(`[HumbleSwap-arccjs] buildO txn after setting on applicationCall:`);
        console.log(`  appCall.foreignApps:`, appCall.foreignApps?.map(a => Number(a)));
      }
