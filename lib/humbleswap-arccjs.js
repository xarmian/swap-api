...
export async function buildSwapTransactions({
  ...
  if (needsApproval) {
    const approveResult = await builder.tokA.arc200_approve(
      algosdk.getApplicationAddress(poolIdNum),
      approvalAmount
    );
    if (approveResult && approveResult.obj) {
      // ARC200 approve doesn't need a payment transaction - only NT200 methods like deposit/createBalanceBox need payments
      ...
      buildO.push({
        ...
        from: sender, // Ensure sender is set
        // No payment needed for ARC200 approve
        note: new TextEncoder().encode(
  ...
  // Transfer 0 to ensure output token balance box exists for the pool
  // Only needed if the pool doesn't already have a balance (balance box might not exist)
  // Check pool's balance first - if it has a balance, the box exists
  ...
  if (!skipWithdraw && outputTokenIdNum >= 0 && !outputHasExchange) {
    ...
    if (withdrawResult && withdrawResult.obj) {
      ...
      // If output is ASA and we need opt-in, add asset transfer fields
      if (outputTokenIdNum > 0) {
  ...
  for (const txn of allTxns) {
    ...
    if (txn.paymentTxn) {
      ...
      const paymentTxn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
        ...
        from: txnSender,
        to: algosdk.getApplicationAddress(txnAppIndex),
        amount: BigInt(txn.payment || 0),
        note: txn.note,
      });
      console.log(`[Txn ${txnIndex}] Payment: ${txnSender} -> ${algosdk.getApplicationAddress(txnAppIndex)}`);
