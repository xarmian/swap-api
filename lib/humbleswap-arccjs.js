...
export async function buildSwapTransactions({
  ...
  if (needsDeposit) {
    if (inputTokenType === 'ASA') {
      ...
      // VOI (native token)
      // Following ulujs pattern: separate createBalanceBox call, then deposit
      // NT200 contracts like wVOI expect this specific transaction structure
      const BalanceBoxCost = 28500n;

      if (DEBUG) {
        console.log(`[HumbleSwap Native Deposit] balA=${balA}, amountBigInt=${amountBigInt}, needsBalanceBox=${balA === 0n}`);
      }

      // Step 1: Create balance box if needed (separate transaction like ulujs)
      if (balA === 0n) {
        const createBalanceBoxResult = await builder.tokA.createBalanceBox(sender);
        if (createBalanceBoxResult && createBalanceBoxResult.obj) {
          buildO.push({
            ...createBalanceBoxResult.obj,
            appIndex: inputWrapped,
            from: sender,
            payment: BalanceBoxCost,
            note: new TextEncoder().encode(
              `Create balance box for ${sender} on ${inputSymbol} contract`
            ),
          });
          if (DEBUG) {
            console.log(`[HumbleSwap Native Deposit] Added createBalanceBox txn with payment=${BalanceBoxCost}`);
          }
        }
      }

      // Step 2: Deposit with payment = just the deposit amount (like ulujs)
      ...
      if (depositResult && depositResult.obj) {
        buildO.push({
          ...
          appIndex: inputWrapped,
          from: sender,
          payment: amountBigInt, // Just the deposit amount, not boxCost + amount
          note: new TextEncoder().encode(
            `Deposit ${new BigNumber(amountBigInt.toString()).dividedBy(
              new BigNumber(10).pow(decA)
            ).toFixed(decA)} ${inputSymbol} to application address ${algosdk.getApplicationAddress(
        ...
        if (DEBUG) {
          console.log(`[HumbleSwap Native Deposit] Added deposit txn with payment=${amountBigInt}`);
        }
      }
    } else {
