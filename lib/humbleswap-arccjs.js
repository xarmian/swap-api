...
export async function buildSwapTransactions({
  ...
  // The swap credits the user's wrapped balance with the ACTUAL pool output (>= minAmountOut).
  // The withdraw step must convert that full actual output back to the underlying token,
  // otherwise the difference (actual - min) is stranded as an invisible wrapped balance.
  // expectedAmountOut is the caller's live quote recomputed for THIS hop's actual input, so it
  // equals the on-chain output at quote reserves and is the correct withdraw amount. It is
  // required whenever a withdraw may occur; we fail loudly rather than silently falling back to
  // minAmountOut (a money bug).
  // Residual (tracked in TASK-41, needs contract-level withdraw-all/clamp): if reserves drift
  // between quoting/simulation and submission, a fixed amount can either over-withdraw (the
  // atomic group fails cleanly, no fund loss) or, on favourable drift, leave a tiny bounded
  // remainder. This is strictly better than the previous minAmountOut behaviour, which stranded
  // (actual - min) on every swap.
  ...
  const withdrawAmountBigInt = BigInt(expectedAmountOut);
  ...
  if (!skipWithdraw && needsWithdraw) {
    ...
    // Withdraw the ACTUAL swap output (expectedAmountOut), not minAmountOut.
    // The swap credits the wrapped balance with the actual output; withdrawing only
