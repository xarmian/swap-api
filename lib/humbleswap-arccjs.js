...
export async function buildSwapTransactions({
  ...
  if (needsApproval) {
    ...
    console.log(`[DEBUG] Approval: arccjs returned ${approveTxns.length} transaction(s) for arc200_approve`);
    console.log(`[DEBUG] Approval: inputWrapped=${inputWrapped}, poolIdNum=${poolIdNum}, approvalAmount=${approvalAmount}`);
    ...
    console.log(`[DEBUG] Approval: Decoded ${decodedTxns.length} transaction(s):`);
    decodedTxns.forEach((txn, idx) => {
      console.log(`[DEBUG] Approval: Txn ${idx}: type=${txn.type} (${txn.type === algosdk.TransactionType.pay ? 'PAYMENT' : txn.type === algosdk.TransactionType.appl ? 'APP_CALL' : 'OTHER'})`);
      if (txn.type === algosdk.TransactionType.pay) {
        const toAddr = typeof txn.to === 'string' ? txn.to : algosdk.encodeAddress(txn.to.publicKey || txn.to);
        console.log(`[DEBUG] Approval: Payment txn: to=${toAddr}, amount=${txn.amount}`);
      }
      if (txn.type === algosdk.TransactionType.appl) {
        console.log(`[DEBUG] Approval: App call txn: appIndex=${txn.appIndex}, appArgs.length=${txn.appArgs?.length || 0}`);
      }
    });
    ...
    console.log(`[DEBUG] Approval: Expected wVOI app address: ${expectedAppAddress}`);
    console.log(`[DEBUG] Approval: Payment txn found: ${paymentTxn !== null}, App call txn found: ${appCallTxn !== null}`);
    ...
    if (paymentTxn) {
      ...
      console.log(`[DEBUG] Approval: Payment txn 'to' address: ${paymentTo}`);
      console.log(`[DEBUG] Approval: Expected app address: ${expectedAppAddress}`);
      console.log(`[DEBUG] Approval: Payment matches expected address: ${paymentTo === expectedAppAddress}`);
      
      if (paymentTo !== expectedAppAddress) {
        console.warn(`[WARNING] Approval payment transaction 'to' address (${paymentTo}) does not match expected app address (${expectedAppAddress}). This may cause the contract to fail!`);
      }
      ...
      console.log(`[DEBUG] Approval: No payment transaction found in arccjs result - creating manual payment transaction`);
      // Some ARC200 tokens like wVOI require a payment transaction before the approval call
      ...
      const BalanceBoxCost = 28500n;
      console.log(`[DEBUG] Approval: Creating manual payment transaction to ${expectedAppAddress} with amount ${BalanceBoxCost} for wVOI approval`);
  ...
  // Log foreignApps for debugging
  console.log(`[DEBUG] Swap transaction foreignApps:`, swapForeignApps);
  console.log(`[DEBUG] Beacon ID: ${beaconId}, App 395642 needed: ${swapForeignApps.includes(395642)}`);
  ...
  console.log(`\n[buildSwapTransactions] Building ${allTxns.length} transaction(s) for HumbleSwap swap:`);
  console.log(`  Pool: ${poolIdNum}, Input: ${inputWrapped}, Output: ${outputWrapped}`);
  console.log(`  Amount In: ${amountBigInt.toString()}, Min Amount Out: ${minAmountOutBigInt.toString()}`);
  console.log(`  Swap Direction: ${actualSwapAForB ? 'A->B' : 'B->A'}`);
  if (skipDeposit) {
    console.log(`  ⚡ Optimization: Skipping deposit (input token from previous hop)`);
  }
  if (skipWithdraw) {
    console.log(`  ⚡ Optimization: Skipping withdraw (output token needed by next hop)`);
  }
  console.log('');
  ...
  for (const txn of allTxns) {
    ...
    if (txn instanceof algosdk.Transaction) {
      // Clear any existing group ID - it will be assigned later when all hops are combined
      txn.group = undefined;
      const txnSummary = getTransactionSummary(txn, txnIndex);
      console.log(`[Txn ${txnIndex}] ${txnSummary}`);
      txns.push(txn);
      txnIndex++;
      continue;
    }
    ...
    if (!txnSender || typeof txnSender !== 'string') {
      console.warn('Invalid sender address in transaction:', txn);
      continue;
    }
    ...
    if (txn.xaid && txn.aamt) {
      ...
      if (txn.to) {
        ...
        console.warn('Invalid destination for asset transfer:', txn);
        continue;
      }
      ...
      console.log(`[Txn ${txnIndex}] Asset Transfer: ${txnSender} -> ${assetTransferTo}`);
      console.log(`  Asset ID: ${txn.xaid}, Amount: ${txn.aamt}`);
    ...
    if (txn.paymentTxn) {
      ...
      if (txn.paymentTxnBase64) {
        ...
        if (typeof originalPaymentTxn.from === 'string') {
          ...
          console.error('[ERROR] Unable to convert paymentFrom address. Type:', typeof originalPaymentTxn.from, 'Value:', originalPaymentTxn.from);
          console.error('[ERROR] paymentFrom structure:', Object.keys(originalPaymentTxn.from || {}));
        ...
        if (typeof originalPaymentTxn.to === 'string') {
          ...
          console.error('[ERROR] Unable to convert paymentTo address. Type:', typeof originalPaymentTxn.to, 'Value:', originalPaymentTxn.to);
          console.error('[ERROR] paymentTo structure:', Object.keys(originalPaymentTxn.to || {}));
          // Try to stringify it to see what it is
          try {
            console.error('[ERROR] paymentTo JSON:', JSON.stringify(originalPaymentTxn.to));
          } catch (e) {
            console.error('[ERROR] Could not stringify paymentTo:', e.message);
          }
          throw new Error(`Invalid paymentTo address: ${originalPaymentTxn.to}`);
        }
        
        // Verify the payment is going to the correct app address
        // The contract checks CurrentApplicationAddress == payment.to, so they must match
        const expectedAppAddress = txn.appIndex ? algosdk.getApplicationAddress(txn.appIndex) : null;
        if (expectedAppAddress && paymentTo !== expectedAppAddress) {
          console.error(`[ERROR] Payment transaction 'to' address (${paymentTo}) does not match expected app address (${expectedAppAddress}). This will cause the contract to fail!`);
          // Still use the original address from arccjs - don't override it
        }
        ...
        // Log with string addresses
        console.log(`[Txn ${txnIndex}] Payment: ${paymentFrom} -> ${paymentTo}`);
        console.log(`  Amount: ${recreatedPaymentTxn.amount} microAlgos (${Number(recreatedPaymentTxn.amount) / 1e6} VOI)`);
        if (expectedAppAddress) {
          console.log(`  Expected app address: ${expectedAppAddress}, Actual: ${paymentTo}, Match: ${paymentTo === expectedAppAddress}`);
        }
      ...
      if (typeof txn.paymentTxn.from === 'string') {
        ...
        console.error('[ERROR] Unable to convert paymentFrom address:', txn.paymentTxn.from);
      ...
      if (typeof txn.paymentTxn.to === 'string') {
        ...
        console.error('[ERROR] Unable to convert paymentTo address:', txn.paymentTxn.to);
      ...
      console.log(`[Txn ${txnIndex}] Payment: ${paymentTxn.from} -> ${paymentTxn.to}`);
      console.log(`  Amount: ${paymentTxn.amount} microAlgos (${Number(paymentTxn.amount) / 1e6} VOI)`);
      if (expectedAppAddress) {
        console.log(`  Expected app address: ${expectedAppAddress}, Actual: ${paymentTo}, Match: ${paymentTo === expectedAppAddress}`);
      }
      ...
      if (!txnAppIndex || typeof txnAppIndex !== 'number') {
        console.warn('Invalid appIndex for payment:', txn);
        continue;
      }
      ...
      paymentTxn.group = undefined; // Clear group ID - will be assigned later
      console.log(`[Txn ${txnIndex}] Payment: ${txnSender} -> ${paymentToAddress}`);
      console.log(`  Amount: ${txn.payment || 0} microAlgos (${Number(txn.payment || 0) / 1e6} VOI)`);
      console.log(`  [DEBUG] Manual payment for app ${txnAppIndex} (wVOI approval)`);
      txns.push(paymentTxn);
      txnIndex++;
    }
    ...
    if (txn.xaid && txn.snd && txn.arcv && !txn.xamt) {
      ...
      console.log(`[Txn ${txnIndex}] Asset Opt-In: ${txn.snd} -> ${txn.arcv}`);
      console.log(`  Asset ID: ${txn.xaid} (opt-in)`);
      txns.push(optInTxn);
      txnIndex++;
    }
    ...
    if (txn.appCallTxn) {
      ...
      console.log('[DEBUG] App call boxes:', {
        boxes: txn.appCallTxn.boxes,
        boxesType: typeof txn.appCallTxn.boxes,
        isArray: Array.isArray(txn.appCallTxn.boxes),
        length: txn.appCallTxn.boxes?.length
      });
      if (txn.appCallTxn.boxes && Array.isArray(txn.appCallTxn.boxes)) {
        for (const box of txn.appCallTxn.boxes) {
          console.log('[DEBUG] Processing box:', {
            box: box,
            appIndex: box?.appIndex,
            appIndexType: typeof box?.appIndex,
            name: box?.name,
            nameType: typeof box?.name,
            nameIsUint8Array: box?.name instanceof Uint8Array
          });
          ...
          if (typeof box.appIndex === 'number') {
            ...
            if (isNaN(appIndex)) {
              console.warn('Invalid box appIndex:', box.appIndex);
              continue;
            }
          } else {
            console.warn('Box missing appIndex:', box);
            continue;
          }
          ...
          if (box.name instanceof Buffer) {
            ...
            try {
              ...
              console.warn('Failed to convert box name to Uint8Array:', box.name, e);
              continue;
            }
          } else {
            console.warn('Box missing name:', box);
            continue;
          }
      ...
      console.log('[DEBUG] App call appArgs:', {
        appArgsLength: appArgs.length,
        appArgs0Hex: appArgs.length > 0 ? Buffer.from(appArgs[0]).toString('hex') : 'none',
        appArgs0Length: appArgs.length > 0 ? appArgs[0].length : 0
      });
      
      if (appArgs.length === 0) {
        console.warn('[WARNING] App call transaction has no appArgs! This will fail.');
      }
      ...
      // Convert address to string for logging (transaction object has address object)
      const appCallFromStr = typeof appCallTxn.from === 'string' ? appCallTxn.from : algosdk.encodeAddress(appCallTxn.from.publicKey);
      console.log(`[Txn ${txnIndex}] App Call: ${appCallFromStr} -> App ${appCallTxn.appIndex}`);
      console.log(`  ${methodName}`);
      if (appCallTxn.foreignApps && appCallTxn.foreignApps.length > 0) {
        console.log(`  Foreign Apps: ${appCallTxn.foreignApps.join(', ')}`);
      }
      if (appCallTxn.foreignAssets && appCallTxn.foreignAssets.length > 0) {
        console.log(`  Foreign Assets: ${appCallTxn.foreignAssets.join(', ')}`);
      }
      if (appCallTxn.boxes && appCallTxn.boxes.length > 0) {
        console.log(`  Boxes: ${appCallTxn.boxes.length} box(es)`);
      }
      ...
      console.log('[DEBUG] Old path app call appArgs:', {
        appArgsLength: appArgs.length,
        appArgs0Hex: appArgs.length > 0 ? Buffer.from(appArgs[0]).toString('hex') : 'none',
      ...
      if (appArgs.length === 0) {
        console.warn(`[WARNING] App call to ${txnAppIndex} has no appArgs! This will fail. Transaction:`, txn);
      ...
      // Log foreignApps for swap transactions to debug
      if (txnAppIndex === 395553) {
        console.log(`[DEBUG] Building swap transaction (395553) with foreignApps:`, foreignApps);
      }
      ...
      const noteText = txn.note ? new TextDecoder().decode(txn.note) : 'No note';
      console.log(`[Txn ${txnIndex}] App Call: ${txnSender} -> App ${txnAppIndex}`);
      console.log(`  Method: ${noteText.substring(0, 50)}${noteText.length > 50 ? '...' : ''}`);
      if (foreignApps.length > 0) {
        console.log(`  Foreign Apps: ${foreignApps.join(', ')}`);
      }
      const finalForeignAssets = txn.foreignAssets || [];
      if (finalForeignAssets.length > 0) {
        console.log(`  Foreign Assets: ${finalForeignAssets.join(', ')}`);
      }
      if (txn.boxes && txn.boxes.length > 0) {
        console.log(`  Boxes: ${txn.boxes.length} box(es)`);
        txn.boxes.forEach((box, idx) => {
          const boxNameStr = Buffer.from(box.name).toString('hex').substring(0, 16);
          console.log(`    Box ${idx + 1}: App ${box.appIndex}, Name: 0x${boxNameStr}...`);
        });
      }
      if (txn.accounts && txn.accounts.length > 0) {
        console.log(`  Accounts: ${txn.accounts.length} account(s)`);
      }
  ...
  console.log(`\n[buildSwapTransactions] Built ${txns.length} transaction(s) total\n`);
