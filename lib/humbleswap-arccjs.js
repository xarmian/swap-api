...
export async function buildSwapTransactions({
  ...
  if (needsDeposit) {
    if (inputTokenType === 'ASA') {
      ...
      if (inputHasExchange) {
        ...
        let assetBalance = 0n;
        try {
          const assetInfo = await algodClient.accountAssetInformation(acc.addr, inputTokenIdNum).do();
          // algosdk 3.x uses camelCase 'assetHolding' instead of 'asset-holding'
          assetBalance = assetInfo.assetHolding?.amount ?? assetInfo["asset-holding"]?.amount ?? 0n;
        } catch (e) {
          // 404 means account hasn't opted into asset - balance is 0
          if (e.status !== 404) {
            throw e;
          }
        }
      ...
      // VOI (native token)
      // Use createUtxns to get the full transaction group (similar to ASA deposits)
      // NT200 contracts like wVOI expect specific transaction group structure

      // Set payment amount for box cost + deposit amount
      const BalanceBoxCost = 28500n;
      let boxCost = balA === 0n ? BalanceBoxCost : 0n;
      builder.tokA.setPaymentAmount(boxCost + amountBigInt);
      ...
      const originalObjectOnly = builder.tokA.objectOnly;
      builder.tokA.objectOnly = false;
      let nativeDepositTxns;
      try {
        nativeDepositTxns = await builder.tokA.createUtxns(depositMethod, [amountBigInt]);
      } finally {
        builder.tokA.objectOnly = originalObjectOnly;
      }

      // Decode transactions to find payment and app call
      const decodedNativeDepositTxns = nativeDepositTxns.map(txn => {
        const decoded = algosdk.decodeUnsignedTransaction(Buffer.from(txn, 'base64'));
        decoded.group = undefined;
        return decoded;
      });

      const nativePaymentTxnIndex = decodedNativeDepositTxns.findIndex(txn => txn.type === algosdk.TransactionType.pay);
      const nativeAppCallTxnIndex = decodedNativeDepositTxns.findIndex(txn => txn.type === algosdk.TransactionType.appl);
      const nativePaymentTxn = nativePaymentTxnIndex >= 0 ? decodedNativeDepositTxns[nativePaymentTxnIndex] : null;
      const nativeAppCallTxn = nativeAppCallTxnIndex >= 0 ? decodedNativeDepositTxns[nativeAppCallTxnIndex] : null;
      const nativePaymentTxnBase64 = nativePaymentTxnIndex >= 0 ? nativeDepositTxns[nativePaymentTxnIndex] : null;

      // Add payment transaction if it exists
      if (nativePaymentTxn) {
        const paymentAmount = nativePaymentTxn.payment?.amount ?? nativePaymentTxn.amount;
        const paymentNote = nativePaymentTxn.payment?.note ?? nativePaymentTxn.note;
        buildO.push({
          appIndex: inputWrapped,
          from: sender,
          payment: BigInt(paymentAmount),
          paymentTxn: nativePaymentTxn,
          paymentTxnBase64: nativePaymentTxnBase64,
          note: paymentNote,
        });
      }

      // Add the deposit app call transaction
      if (nativeAppCallTxn) {
        buildO.push({
          appIndex: inputWrapped,
          from: sender,
          appCallTxn: nativeAppCallTxn,
          note: new TextEncoder().encode(
            `Deposit ${new BigNumber(amountBigInt.toString()).dividedBy(
              new BigNumber(10).pow(decA)
            ).toFixed(decA)} ${inputSymbol} to application address ${algosdk.getApplicationAddress(
  ...
  if (needsApproval) {
    ...
    if (paymentTxn) {
      ...
      // In algosdk 3.x, payment receiver is at paymentTxn.payment.receiver
      let paymentTo;
      const receiver = paymentTxn.payment?.receiver ?? paymentTxn.to;
      if (typeof receiver === 'string') {
        paymentTo = receiver;
      } else if (receiver instanceof algosdk.Address) {
        paymentTo = receiver.toString();
      } else if (receiver && receiver.publicKey) {
        paymentTo = new algosdk.Address(receiver.publicKey).toString();
      } else if (Buffer.isBuffer(receiver)) {
        paymentTo = new algosdk.Address(new Uint8Array(receiver)).toString();
      } else if (receiver instanceof Uint8Array) {
        paymentTo = new algosdk.Address(receiver).toString();
      } else if (Array.isArray(receiver)) {
        paymentTo = new algosdk.Address(new Uint8Array(receiver)).toString();
      } else {
        throw new Error(`Invalid paymentTo address in approval payment transaction: ${receiver}`);
      }

      // Get payment amount - in algosdk 3.x it's at payment.amount
  ...
  if (!skipWithdraw && needsWithdraw) {
    ...
    let withdrawTxns;
    try {
      withdrawTxns = await builder.tokB.createUtxns(withdrawMethod, [minAmountOutBigInt]);
    } finally {
