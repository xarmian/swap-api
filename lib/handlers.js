import { calculateOptimalSplit, findOptimalMultiHopRoute, createPoolInfoCache, getPlatformFeeConfig } from './quotes.js';
  ...
  try {
    ...
    if (poolId) {
      ...
      if (matchingPools.length === 0) {
        ...
        // Evaluating them here is therefore pure duplicate quoting of the direct
        // pairs that cannot change selection or amounts -- with ONE exception: a
        // MISCONFIGURED platform fee (PLATFORM_FEE_BPS > 10000, i.e. a fee
        // exceeding 100% of the optimization gain). calculateOptimalSplit selects
        // its winner by RAW output and only then subtracts feeAmount =
        // gain * feeBps / 10000; for feeBps <= 10000 that leaves the split >= the
        // best single pool, so a fee-free 1-hop concrete single-pool route can
        // only tie. But for feeBps > 10000 the fee-adjusted split can fall BELOW
        // the best single pool's raw quote, so a 1-hop concrete route (which
        // carries no platform fee) could legitimately beat the direct split and
        // must NOT be dropped. So we only prune the 1-hop duplicates when the fee
        // provably cannot invert that ordering; under the broken-fee config we
        // fall back to evaluating every route exactly as before -- changing
        // selection/amounts in NO configuration. (TASK-23)
        const { feeBps, feeAddress } = getPlatformFeeConfig();
        const platformFeeMayInvertSelection = feeBps > 10000 && Boolean(feeAddress);
        const allRoutes = findRoutes(inputToken, outputToken, 2, dex);
        const multiHopRoutes = platformFeeMayInvertSelection
          ? allRoutes
          : allRoutes.filter(route => route.hops > 1);
    ...
    // (calculateMultiHopQuote / calculateOptimalRouteSplit merge each hop's
    // and each route's own skippedPools up onto the returned quote object).
    let skippedPools = [];
