...
import { getPoolConfigById, findMatchingPools, findRoutes, getDiscoveryStatus, MAX_ROUTE_COMBINATIONS } from './config.js';
...
// line without ever exposing internals over the wire (TASK-27). This is the
// handler for the REAL production error paths - handleQuote/handleUnwrap
// catch and respond to most request-time errors right here, so index.js's
// own outer try/catch (with its own copy of this same helper) rarely fires.
  ...
  try {
    ...
    if (poolId) {
      ...
      if (matchingPools.length === 0) {
        ...
        if (routes.length > 0) {
          ...
          try {
            const optimalResult = await findOptimalMultiHopRoute(
              ...
              slippage,
              address,
              // TASK-26: cap the per-request route-combination fan-out at
              // MAX_ROUTE_COMBINATIONS (env-tunable, default 10) instead of
              // relying on findOptimalMultiHopRoute's own default, so the
              // cap is explicit and effective at this real /quote call site.
        ...
        // calculateOptimalSplit caps the platform fee at the realized gain
        // (see lib/quotes.js), so the fee-adjusted split is ALWAYS >= the best
        // single pool -- even at the maximum valid PLATFORM_FEE_BPS of 10000
        // (values above 10000 are hard-rejected at config read, TASK-56). A
    ...
    if (isDirectRoute) {
      try {
        ...
        if (multiHopQuote) {
          ...
          return internalError(res, splitError, errorMsg, {
            label: errorMsg,
            discoveryStatus: discoveryStatusAtRequestStart
          });
        }
      }
    }
    ...
    if (isDirectRoute && splitResult) {
      ...
      return internalError(res, new Error(errorMsg), errorMsg, {
        label: 'Failed to determine route',
        discoveryStatus: discoveryStatusAtRequestStart
      });
    }
    ...
    if (address) {
      try {
        ...
        // Still a genuine internal failure (algod/SDK error building the
        // swap group) even though the response below is HTTP 200 - the quote
        // itself succeeded, but unsignedTransactions comes back empty. Tag
        // and log it the same client-safe way as every other internal
        // failure (TASK-27): never put txnError.message on the wire, only a
        // correlatable errorId.
        const txnErrorId = randomUUID();
        console.error(`[${txnErrorId}] Error building swap transactions:`, txnError);
        ...
        const response = {
          ...
          // Surface that transaction building failed here instead of hardcoding null -
          // unsignedTransactions is already empty in this path, but a client checking
          // simulationError alone should still see a signal rather than a false "no error"
          // reading. The raw txnError.message is deliberately NOT included here (TASK-27) -
          // it's logged server-side above, tagged with errorId, for correlation instead.
          simulationError: 'Failed to generate swap transactions',
    ...
    internalError(res, error, 'Error generating quote', {
      discoveryStatus: discoveryStatusAtRequestStart || getDiscoveryStatus()
    });
  }
}
...
export async function handleUnwrap(req, res) {
  try {
    ...
    try {
      ...
      // Trust an explicit isClientError flag set at the validation throw site
      // (lib/transactions.js's buildBatchUnwrapTransactions/buildSingleUnwrapTxn)
      // rather than guessing from the message TEXT. A substring match like
      // "invalid" can also appear inside a genuine internal/algod error (e.g.
      // "invalid Box reference" from a simulation failure), which would wrongly
      // echo that internal detail to the client as if it were a safe
      // validation message (TASK-27). Anything not explicitly tagged defaults
      // to the safe (internal/500, genericized) branch below.
      ...
      if (status === 400) {
        // buildBatchUnwrapTransactions' own thrown text (matched above by the
        // isClientError substring check) is a deliberate, safe client-facing
        // validation message - mirrors index.js's intentional 4xx guidance
        // strings and must stay verbatim, not be genericized (TASK-27).
        console.error('Error building unwrap transactions:', error);
        return res.status(400).json({
          error: 'Invalid unwrap request',
          message: error.message,
          // Keep this field present and honest on this validation path - a caller checking
          // simulationError alone (as it does on the success path) should still see the reason.
          simulationError: error.message
        });
      }
      // A genuine (non-validation) failure: never echo error.message here -
      // fall through to the same client-safe generic shape as every other
      // internal/500 response.
      return internalError(res, error, 'Error building unwrap transactions', {
        label: 'Failed to generate unwrap transactions'
      });
    }
  } catch (error) {
    // Log error to Supabase (fire and forget)
    ...
    return internalError(res, error, 'Unexpected error handling unwrap request');
  }
}
