  ...
  try {
    ...
    if (isDirectRoute && splitResult) {
      ...
      if (multiHopQuote.splitDetails && Array.isArray(multiHopQuote.splitDetails)) {
        ...
        // IMPORTANT: For intermediate hops (not the first), apply a small buffer to the input amount
        // to account for minor rounding/timing differences between quote and execution.
        // We use 99.9% of the previous hop's expected output (0.1% buffer) rather than the full
        // slippage-adjusted minOutput to avoid being overly conservative.
        // This ensures we don't try to send more than we'll likely receive from the previous hop.
        const INTERMEDIATE_HOP_BUFFER = 999n; // 99.9% (divide by 1000)
        let previousHopExpectedOutput = null;

        for (let hopIndex = 0; hopIndex < multiHopQuote.hopQuotes.length; hopIndex++) {
          const hopQuote = multiHopQuote.hopQuotes[hopIndex];
          ...
          if (hopQuote.isSplit && hopQuote.splitDetails) {
            ...
            for (const split of hopQuote.splitDetails) {
              // For non-first hops, use previous hop's buffered expected output as input amount
              let adjustedInputAmount = split.inputAmount;
              if (!isFirstHop && previousHopExpectedOutput) {
                // Apply 0.1% buffer: previousOutput * 999 / 1000
                adjustedInputAmount = ((BigInt(previousHopExpectedOutput) * INTERMEDIATE_HOP_BUFFER) / 1000n).toString();
              }

              splitDetails.push({
                poolCfg: split.poolCfg,
                amount: adjustedInputAmount,
            ...
            // Track the combined expected output for split hops (sum of all splits)
            const combinedExpectedOutput = hopQuote.splitDetails.reduce(
              (sum, s) => BigInt(sum) + BigInt(s.outputAmount), 0n
            ).toString();
            previousHopExpectedOutput = combinedExpectedOutput;
          } else {
            // Single pool hop
            // For non-first hops, use previous hop's buffered expected output as input amount
            let adjustedInputAmount = hopQuote.inputAmount;
            if (!isFirstHop && previousHopExpectedOutput) {
              // Apply 0.1% buffer: previousOutput * 999 / 1000
              adjustedInputAmount = ((BigInt(previousHopExpectedOutput) * INTERMEDIATE_HOP_BUFFER) / 1000n).toString();
            }
            ...
            previousHopExpectedOutput = hopQuote.outputAmount;
          }
        }
      }
