    ...
    if (simulationResult && simulationResult.simulateResponse) {
      ...
      for (let i = 0; i < txns.length; i++) {
        ...
        if (txn.type === 'appl') {
          ...
          const appIdx = appCall.appIndex || txn.appIndex;
          console.log(`[CollectResources] Txn ${i} (app ${appIdx}): foreignApps=${txnForeignApps.map(a => Number(a))}`);
        ...
        for (let verifyIteration = 0; verifyIteration < MAX_VERIFY_ITERATIONS; verifyIteration++) {
          try {
            ...
            if (newResourcesFound === 0) {
              ...
              if (DEBUG) console.log(`[Verification] No new resources found but verification failed: ${verifyFailure.substring(0, 200)}`);
              break;
            }
        ...
        for (let i = 0; i < allTransactions.length; i++) {
          ...
          if (txn.type === 'appl') {
            ...
            if (appCall.accounts && appCall.accounts.length > 4) {
              console.log(`[Warning] Txn ${i} has ${appCall.accounts.length} accounts, trimming to 4`);
            ...
            if (totalReferences > 8) {
              ...
              if (appCall.boxes && appCall.boxes.length > maxBoxes) {
                console.log(`[Warning] Txn ${i} has ${appCall.boxes.length} boxes, trimming to ${maxBoxes}`);
