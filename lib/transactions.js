...
async function buildSwapTransactions(splitDetails, inputToken, outputToken, slippage, address, platformFee = null) {
      ...
      if (simulationResult && simulationResult.simulateResponse) {
        ...
        for (let i = 0; i < allTransactions.length; i++) {
          ...
          if (needsRebuild) {
            ...
            // Log original fee before rebuilding
            const originalFee = txn.fee;
            const originalBoxes = txn.boxes?.length || 0;
            if (originalFee > 10000) {
              console.log(`[buildSwapTransactions] WARNING: Transaction ${i} (App ${txn.appIndex}) has high fee before rebuild: ${originalFee} microAlgos (boxes: ${originalBoxes})`);
            }
            ...
            if (boxesToAdd && boxesToAdd.length > 0) {
              ...
              if (originalFee !== finalFee) {
                console.log(`[buildSwapTransactions] Corrected fee for txn ${i} (App ${txn.appIndex}): ${originalFee} -> ${finalFee} microAlgos (${finalBoxesCount} boxes)`);
              }
            } else {
              ...
              if (rebuiltTxn.fee !== 1000) {
                ...
                if (originalFee !== 1000) {
                  console.log(`[buildSwapTransactions] Corrected fee for txn ${i} (App ${txn.appIndex}): ${originalFee} -> 1000 microAlgos (no boxes)`);
                }
              }
            }
  ...
  for (let i = 0; i < allTransactions.length; i++) {
    ...
    if (txn.type === 'pay' && txn.amount !== undefined) {
      totalFees += Number(txn.amount);
      console.log(`    [DEBUG] Including payment amount: ${txn.amount} microAlgos (total now: ${totalFees})`);
    }
    
    // Debug fee calculation for app call transactions with suspiciously high fees
    if (txn.type === 'appl' && fee > 10000) {
      const baseFee = 1000;
      const boxFee = (txn.boxes?.length || 0) * 2500;
      const expectedFee = baseFee + boxFee;
      const actualFee = fee;
      
      console.log(`    [DEBUG] Investigating high fee for txn ${i} (App ${txn.appIndex}):`);
      console.log(`      - txn.fee property: ${txn.fee} (type: ${typeof txn.fee})`);
      console.log(`      - txn.suggestedParams?.fee: ${txn.suggestedParams?.fee} (type: ${typeof txn.suggestedParams?.fee})`);
      console.log(`      - txn.suggestedParams exists: ${!!txn.suggestedParams}`);
      console.log(`      - Boxes count: ${txn.boxes?.length || 0}`);
      console.log(`      - Foreign Apps count: ${txn.foreignApps?.length || 0}`);
      console.log(`      - Foreign Assets count: ${txn.foreignAssets?.length || 0}`);
      console.log(`      - Accounts count: ${(txn.accounts || []).length}`);
      console.log(`      - Expected fee (base ${baseFee} + boxes ${boxFee}): ${expectedFee} microAlgos`);
      console.log(`      - Actual fee being used: ${actualFee} microAlgos`);
      console.log(`      - Difference: ${actualFee - expectedFee} microAlgos`);
      
      // Check if fee might be stored in a different property
      if (txn.fee !== actualFee) {
        console.log(`      - WARNING: txn.fee (${txn.fee}) differs from calculated fee (${actualFee})`);
      }
      if (txn.suggestedParams && txn.suggestedParams.fee !== actualFee) {
        console.log(`      - WARNING: suggestedParams.fee (${txn.suggestedParams.fee}) differs from calculated fee (${actualFee})`);
      }
    }
  }
  console.log(`  Total fees: ${totalFees} microAlgos`);
  console.log('');
