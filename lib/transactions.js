 ...
 * the split's reported `minOutput` with NO extra buffer: the minimum shown to the
 * user (Σ split.minOutput in the response) has to be exactly the minimum the
 * contract enforces on-chain (TASK-6). A prior bug applied a 0.99 haircut here,
 * so the user could receive ~1% below the reported floor undisclosed. Slippage is
...
function logResourceSummary(label, transactions) {
  ...
  for (let i = 0; i < transactions.length; i++) {
    ...
    if (txn.type === 'appl') {
      ...
      console.log(`  Txn ${i} (app ${txn.appIndex}): apps=${apps} assets=${assets} accts=${accounts} boxes=${boxes} => ${total}/8 [${status}]`);
      totalSlots += 8;
      usedSlots += total;
    }
  }
...
async function buildBatchUnwrapTransactions({ address, items }) {
    ...
    if (simulationResult && simulationResult.simulateResponse) {
      ...
      for (let i = 0; i < txns.length; i++) {
        ...
        if (txn.type === 'appl') {
          ...
          console.log(`[CollectResources] Txn ${i} (app ${appCall.appIndex}): foreignApps=${txnForeignApps.map(a => Number(a))}`);
      ...
      if (DEBUG) {
        console.log(`[Distribution] Pre-check: need ${totalNeeded} slots, have ${availableSlots} available (${allUniqueBoxes.length} boxes, ${allUniqueForeignApps.size} apps, ${allUniqueForeignAssets.size} assets, ${allUniqueAccounts.size} accounts)`);
      }
  ...
  for (let i = 0; i < txns.length; i++) {
    ...
    if (DEBUG) {
      console.log(`[Rebuild] Txn ${i} (app ${appCall.appIndex}): rebuilding with ${appCallBoxes.length} boxes, ${appCallForeignApps.length} foreignApps, ${appCallForeignAssets.length} foreignAssets, ${appCallAccounts.length} accounts`);
    }
    ...
    console.log(`[Rebuild] Txn ${i} FINAL: appIndex=${finalAppCall.appIndex}, boxes=${(finalAppCall.boxes || []).map(b => ({ app: Number(b.appIndex), name: Buffer.from(b.name).toString('hex').substring(0, 20) }))}, foreignApps=${(finalAppCall.foreignApps || []).map(a => Number(a))}`);
  }
...
async function buildSwapTransactions(splitDetails, inputToken, outputToken, slippage, address, platformFee = null) {
  ...
  for (let i = 0; i < splitDetails.length; i++) {
    ...
    if (dex === 'nomadex') {
      ...
      // Enforce the exact quoted minimum on-chain: the minimum reported to the
      // user (Σ split.minOutput) must equal the minimum enforced by the contract.
      // Slippage is already applied when computing split.minOutput, so no hidden margin here.
      ...
      // pair matches neither orientation (that would build the swap against the
      // wrong reserves/direction). Mirrors the quote-path guard in quotes.js.
      ...
      const decodedTxns = await buildHumbleswapSwapTransactionsArccjs({
        ...
        amountIn: amountBigInt.toString(),
        minAmountOut: enforcedMinAmountOut(minOutput),
        // Expected (actual) raw pool output for THIS hop's actual input amount. The swap
        // credits the wrapped balance with this amount, so the withdraw must convert it in
        // full (not just minAmountOut), otherwise the difference is stranded as an invisible
        // wrapped balance.
        ...
        // withdrawn underlying token). Residual drift risk is tracked in TASK-41.
  ...
  if (shouldSimulateGroup && allTransactions.length > 0) {
    try {
      ...
      for (let simIteration = 0; simIteration < MAX_SIMULATION_ITERATIONS; simIteration++) {
        ...
        console.log(`[Simulation] Failed on iteration ${simIteration + 1}: ${failureMessage?.substring(0, 300) || 'Unknown error'}`);
        simulationErrorMessage = failureMessage || 'Unknown simulation error';
      ...
      if (lastSimulationPassed && simulationResult) {
        ...
        if (DEBUG) {
          console.log(`[Pre-Verification] Discovered from simulation: apps=${preVerifyApps.size}, assets=${preVerifyAssets.size}, accounts=${preVerifyAccounts.size}, holdings=${preVerifyHoldings.length}, boxes=${simulationResult.boxReferences?.length || 0}`);
        }
        ...
        for (let verifyIteration = 0; verifyIteration < MAX_VERIFY_ITERATIONS; verifyIteration++) {
          try {
            ...
            if (!verifyFailure) {
              ...
              console.log(`[Verify] Passed${iterStr}: ${allTransactions.length} txns, ${totalApps} apps, ${totalAssets} assets, ${totalAccounts} accounts, ${totalBoxes} boxes`);
              if (DEBUG) {
                for (let t = 0; t < allTransactions.length; t++) {
                  const txn = allTransactions[t];
                  if (txn.type === 'appl') {
                    const ac = txn.applicationCall || txn;
                    console.log(`[Verification] Final Txn ${t} (app ${txn.appIndex}): foreignAssets=${(ac.foreignAssets || []).map(a => Number(a))}, accounts=${(ac.accounts || []).map(a => a.toString ? a.toString() : a)}`);
                  }
                }
              }
              break;
            ...
            if (DEBUG) {
              console.log(`[Verification] Iteration ${verifyIteration + 1} failure: ${verifyFailure.substring(0, 300)}`);
            }
      ...
      if (simulationResult && simulationResult.simulateResponse) {
        ...
        for (const appLocal of groupAppLocals) {
          ...
          uniqueForeignApps.add(appId);
          uniqueAccounts.add(accountAddr);
          console.log(`[Simulation] Discovered appLocal: app=${appId}, account=${accountAddr}`);
        }
        ...
        for (const assetHolding of groupAssetHoldings) {
          ...
          uniqueForeignAssets.add(assetId);
          uniqueAccounts.add(accountAddr);
          console.log(`[Simulation] Discovered assetHolding: asset=${assetId}, account=${accountAddr}`);
        }
        ...
        for (let i = 0; i < txnResults.length && i < allTransactions.length; i++) {
          ...
          for (const appLocal of txnAppLocals) {
            ...
            uniqueForeignApps.add(appId);
            uniqueAccounts.add(accountAddr);
            console.log(`[Simulation] Txn ${i} discovered appLocal: app=${appId}, account=${accountAddr}`);
          }
          ...
          for (const assetHolding of txnAssetHoldings) {
            ...
            uniqueForeignAssets.add(assetId);
            uniqueAccounts.add(accountAddr);
            console.log(`[Simulation] Txn ${i} discovered assetHolding: asset=${assetId}, account=${accountAddr}`);
          }
        }
        ...
        for (let i = 0; i < allTransactions.length; i++) {
          ...
          if (txn.type === 'appl') {
            ...
            if (existingForeignApps.length > 0) {
              console.log(`[CollectExisting] Txn ${i} (app ${appCall.appIndex}): foreignApps=${existingForeignApps.map(a => Number(a))}`);
        ...
        for (const appLocal of appLocalPairs) {
          ...
          if (pairInGroup) {
            console.log(`[AppLocal] Pair already in group: app=${appId}, account=${accountAddr}`);
            continue;
          }
          ...
          for (const { txn, idx } of appCallTxns) {
            ...
            if (currentReferences + slotsNeeded <= 8 && accounts.length + needsAccountSlot <= 4) {
              ...
              console.log(`[AppLocal] Added pair to txn ${idx}: app=${appId}, account=${accountAddr}`);
              added = true;
              break;
            }
          }
          ...
          if (!added) {
            console.log(`[AppLocal] WARN: Could not add pair to any transaction: app=${appId}, account=${accountAddr}`);
          }
        }
        ...
        for (const assetHolding of assetHoldingPairs) {
          ...
          if (pairInGroup) {
            console.log(`[AssetHolding] Pair already in group: asset=${assetId}, account=${accountAddr}`);
            continue;
          }
          ...
          try {
            for (const { txn, appCall } of appCallTxns) {
              ...
              if (appIndex) {
                ...
                if (poolAppAddress === accountAddr) {
                  targetPoolAppIndex = Number(appIndex);
                  console.log(`[AssetHolding] Account ${accountAddr} matches pool app ${targetPoolAppIndex}`);
                  break;
                }
              }
            }
          ...
          if (targetPoolAppIndex !== null) {
            for (const { txn, idx } of appCallTxns) {
              ...
              if (currentReferences + slotsNeeded <= 8 && accounts.length + needsAccountSlot <= 4) {
                ...
                console.log(`[AssetHolding] Added pair to MATCHING pool txn ${idx} (app ${targetPoolAppIndex}): asset=${assetId}, account=${accountAddr}`);
                added = true;
                break;
              }
            }
          ...
          if (!added) {
            for (const { txn, idx } of appCallTxns) {
              ...
              if (currentReferences + slotsNeeded <= 8 && accounts.length + needsAccountSlot <= 4) {
                ...
                console.log(`[AssetHolding] Added pair to fallback txn ${idx}: asset=${assetId}, account=${accountAddr}`);
                added = true;
                break;
              }
            }
          ...
          if (!added) {
            console.log(`[AssetHolding] WARN: Could not add pair to any transaction: asset=${assetId}, account=${accountAddr}`);
          }
        }
        ...
        for (let i = 0; i < allTransactions.length; i++) {
          ...
          if (txn.type === 'appl') {
            ...
            if (appCall.accounts && appCall.accounts.length > 4) {
              console.error(`[Validation] Txn ${i} (app ${txn.appIndex}) EXCEEDS ACCOUNT LIMIT: ${appCall.accounts.length}/4 accounts - trimming`);
            ...
            if (totalReferences > 8) {
              ...
              console.error(`[Validation] Txn ${i} (app ${txn.appIndex}) EXCEEDS REFERENCE LIMIT: apps=${currentForeignApps} assets=${currentForeignAssets} accts=${currentAccounts} boxes=${currentBoxes} => ${totalReferences}/8`);
        ...
        for (let i = 0; i < allTransactions.length; i++) {
          ...
          if (needsRebuild) {
            ...
            console.log(`[Rebuild] Txn ${i} (app ${txn.appIndex}): foreignApps from applicationCall=${foreignApps.map(a => Number(a))}`);
            if (isNomadexTxn) {
            ...
            if (foreignApps && foreignApps.length > 0) {
              ...
              console.log(`[Rebuild] Txn ${i}: Set applicationCall.foreignApps=${rebuiltAppCall.foreignApps.map(a => Number(a))}`);
            }
...
export { buildSwapTransactions, buildBatchUnwrapTransactions, enforcedMinAmountOut };
