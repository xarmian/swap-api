...
import { getTokenMetaFromConfig, getUnderlyingForWrapped, getDiscoveryStatus } from './config.js';
...
async function buildSingleUnwrapTxn({ address, wrappedTokenId, amount, suggestedParams: suggestedParamsIn = null }) {
  ...
  if (!unwrapInfo || !unwrapInfo.unwrapSupported) {
    // lib/config.js's wrapped-token cache is only built from pools that
    // finished discovery (TASK-19) - a token that legitimately belongs to a
    // still-failed/pending-retry pool would otherwise be missing here too,
    // which is a transient server-side gap, NOT a client mistake, and
    // retrying later may resolve it. Only tag this as a client error when
    // discovery is otherwise complete (no pools currently failed/pending).
    if (getDiscoveryStatus().failedPools.length > 0) {
      throw new Error(`Wrapped token ${wrappedTokenId} is not currently recognized as unwrap-capable (pool discovery incomplete; retrying later may resolve this)`);
    }
    throw clientError(`Wrapped token ${wrappedTokenId} is not recognized as unwrap-capable`);
  }
  if (unwrapInfo.underlyingId == null) {
    // unwrapSupported can be true while underlyingId is still null/unresolved
    // (lib/config.js's wrapped-token cache can register a token as
    // unwrap-supported before a pool config has supplied its underlying
    // asset id) - a server-side config/discovery gap, NOT something the
    // client controls, so this is deliberately left untagged (a genericized
    // 500), unlike the client-input checks above/below.
    throw new Error(`Wrapped token ${wrappedTokenId} has no resolved underlying token (config incomplete)`);
  }
  ...
  if (inferredUnderlyingType !== 'native' && inferredUnderlyingType !== 'ASA') {
    // Same rationale: an 'unknown' underlyingType stems from a negative/invalid
    // underlyingId in config data (see lib/config.js), not client input -
    // left untagged (genericized 500).
...
async function buildBatchUnwrapTransactions({ address, items }) {
  ...
  // A truthy but malformed address (bad length/checksum) previously reached
  // algosdk's transaction builder below undetected, throwing a plain,
  // untagged SDK error that got genericized as an internal 500 instead of
  // being reported as the client input mistake it actually is (TASK-27;
  // mirrors index.js's existing isValidAddress check for /quote's address).
  if (!algosdk.isValidAddress(address)) {
    throw clientError(`Invalid address: must be a valid Algorand/Voi address`);
  }
