...
async function buildBatchUnwrapTransactions({ address, items }) {
  ...
  // Algorand/Voi transaction groups are capped at 16 transactions. Each unwrap item becomes
  // exactly one application call transaction, so validate up front rather than letting group
  // assignment/simulation fail obscurely deep inside resource discovery.
  const MAX_UNWRAP_GROUP_SIZE = 16;
  if (items.length > MAX_UNWRAP_GROUP_SIZE) {
    throw new Error(`Too many items: ${items.length} exceeds the maximum transaction group size of ${MAX_UNWRAP_GROUP_SIZE}`);
  }
      ...
      for (const appId of allUniqueForeignApps) {
        try {
          ...
          if (appInfo.application && appInfo.application.params && appInfo.application.params.creator) {
            // algosdk 3.x indexer responses can decode `creator` as an Address object rather than
            // a base32 string - normalize here so downstream `Address.fromString(creatorAccount)`
            // calls (which require a string) don't throw "expected string, got object".
            ...
            if (!verifyFailure) {
              ...
              if (DEBUG) {
                for (let t = 0; t < allTransactions.length; t++) {
                  ...
                  if (txn.type === 'appl') {
                    const ac = txn.applicationCall || txn;
                    console.log(`[Verification] Final Txn ${t} (app ${ac.appIndex}): foreignAssets=${(ac.foreignAssets || []).map(a => Number(a))}, accounts=${(ac.accounts || []).map(a => a.toString ? a.toString() : a)}`);
                  }
                }
              }
              break;
      ...
      if (simulationResult && simulationResult.simulateResponse) {
        ...
        for (let i = 0; i < txnResults.length && i < allTransactions.length; i++) {
          ...
          if (txnApps.length > 0 || txnAssets.length > 0 || txnAccounts.length > 0) {
            console.log(`[Simulation] Txn ${i} (app ${appCall.appIndex}) unnamedResourcesAccessed: apps=${JSON.stringify(txnApps)}, assets=${JSON.stringify(txnAssets)}, accounts=${JSON.stringify(txnAccounts)}`);
          }
        ...
        for (let i = 0; i < allTransactions.length; i++) {
          ...
          if (needsRebuild) {
            ...
            console.log(`[Rebuild] Txn ${i} (app ${appCall.appIndex}): foreignApps from applicationCall=${foreignApps.map(a => Number(a))}`);
            if (isNomadexTxn) {
