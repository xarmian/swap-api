...
async function buildSwapTransactions(splitDetails, inputToken, outputToken, slippage, address, platformFee = null) {
  ...
  // For multi-hop routes, we need to build all transactions first without simulation,
  // then simulate the entire group together to get proper box references
  // For single-hop routes, we can use swap.js's internal simulation (backward compatibility)
  const shouldSimulateGroup = isMultiHop;
