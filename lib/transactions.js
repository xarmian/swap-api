      ...
      for (let idx = 0; idx < poolTransactions.length; idx++) {
        ...
        if (txn.type === 'appl' && inputTokenType === 'ARC200' && inputTokenNum !== 0 && txn.appIndex === inputTokenNum) {
          ...
          if (txn.boxes && txn.boxes.length > 0) {
            ...
            // Manually set boxes after creation - in algosdk 3.x, boxes are under applicationCall
          ...
          try {
            ...
            for (const match of holdingMatches) {
              ...
              if (assetId > 0 && !cumulativeAssetsFromErrors.has(assetId)) {
                ...
                console.log(`[Verification] Discovered missing asset from holding: ${assetId}`);
              }
              if (accountAddr && !cumulativeAccountsFromErrors.has(accountAddr)) {
                cumulativeAccountsFromErrors.add(accountAddr);
                newlyDiscoveredAccounts.add(accountAddr);
            ...
            for (const match of invalidBoxMatches) {
              ...
              if (boxNameHex) {
                ...
                if (!boxAppId) {
                  // Try to extract from transaction context in the error message
                  // Look for patterns like "pc=XXXX" after the box reference which might help identify the app
                  console.warn(`[Verification] Box reference without app ID: 0x${boxNameHex.substring(0, 20)}...`);
                  continue;
                }
                ...
                if (!alreadyHave) {
                  ...
                  console.log(`[Verification] Discovered missing box: app=${boxAppId}, name=0x${boxNameHex.substring(0, 20)}...`);
                }
              }
            }

            const newResourcesFound = newlyDiscoveredApps.size + newlyDiscoveredAssets.size + newlyDiscoveredAccounts.size + newlyDiscoveredBoxes.length;
            ...
            for (const appId of newlyDiscoveredApps) {
              ...
              for (const { appCall } of appCallTxnsForVerify) {
                ...
                if (currentRefs < 8 && !alreadyHas) {
                  ...
                  distributed = true;
                  console.log(`[Verification] Distributed app ${appId} to transaction`);
                  break;
                }
              }
              if (!distributed) {
            ...
            for (const assetId of newlyDiscoveredAssets) {
              ...
              for (const { appCall } of appCallTxnsForVerify) {
                ...
                if (currentRefs < 8 && !alreadyHas) {
                  ...
                  distributed = true;
                  console.log(`[Verification] Distributed asset ${assetId} to transaction`);
                  break;
                }
              }
              if (!distributed) {
            ...
            for (const accountAddr of newlyDiscoveredAccounts) {
              ...
              for (const { appCall } of appCallTxnsForVerify) {
                ...
                if (currentRefs < 8 && currentAccounts < 4 && !alreadyHas) {
                  ...
                  distributed = true;
                  console.log(`[Verification] Distributed account ${accountAddr} to transaction`);
                  break;
                }
              }
              if (!distributed) {
            ...
            for (const box of newlyDiscoveredBoxes) {
              ...
              for (const { appCall } of appCallTxnsForVerify) {
                ...
                if (currentRefs + additionalRefs <= 8) {
                  ...
                  distributed = true;
                  console.log(`[Verification] Distributed box app=${box.appIndex} to transaction`);
                  break;
                }
              }
              if (!distributed) {
      ...
      if (simulationResult && simulationResult.simulateResponse) {
        ...
        for (const split of splitDetails) {
          ...
          if (dex === 'nomadex') {
            ...
            console.log(`[HumbleSwap Resources] inputNum=${inputNum}, inputWrappedNum=${inputWrappedNum}, inputType=${inputType}, poolId=${poolId}`);
            if (inputType === 'ARC200' && inputWrappedNum > 0) {
              ...
              console.log(`[HumbleSwap] Pool balance box key: ${boxKey.substring(0, 50)}..., exists=${uniqueBoxes.has(boxKey)}`);
              if (!uniqueBoxes.has(boxKey)) {
                ...
                console.log(`[HumbleSwap] Added pool's balance box for ARC200 input token ${inputWrappedNum}`);
              }
              ...
              if (!uniqueBoxes.has(userBoxKey)) {
                ...
                console.log(`[HumbleSwap] Added user's balance box for ARC200 input token ${inputWrappedNum}`);
              }
            }
          }
        }
        ...
        for (let i = 0; i < allTransactions.length; i++) {
          ...
          if (needsRebuild) {
            ...
            const rebuiltTxn = algosdk.makeApplicationCallTxnFromObject(txnObj);
            // In algosdk 3.x, boxes/accounts are under applicationCall
            ...
            if (txnObj.accounts && txnObj.accounts.length > 0) {
              ...
              if (existingAccounts.length !== txnObj.accounts.length) {
                rebuiltAppCall.accounts = txnObj.accounts;
              }
            }
