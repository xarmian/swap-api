...
async function calculateOptimalSplit(matchingPools, inputToken, outputToken, totalAmount, slippage, address = '') {
  ...
  // Pre-fetch pool info for ALL pools (both HumbleSwap and Nomadex). A
  // failure here is only tentatively recorded: it does NOT by itself mean
  // the pool is unusable (calculateQuoteForPool re-fetches internally when
  // cachedPoolInfo is falsy), so it's reconciled below against whether the
  // pool's actual QUOTE attempt (not just the info pre-fetch) ever succeeds.
  await Promise.all(
  ...
  if (numPools === 2) {
    ...
    const fullQuote1 = await calculateQuoteForPool(poolCfg1, inputToken, outputToken, totalAmount, slippage, address, cachedInfo1).then((q) => {
    ...
    const fullQuote2 = await calculateQuoteForPool(poolCfg2, inputToken, outputToken, totalAmount, slippage, address, cachedInfo2).then((q) => {
    ...
    if (!resolvedInfo1) {
      resolvedInfo1 = poolCfg1.dex === 'nomadex'
        ? await getNomadexPoolInfo(Number(poolCfg1.poolId), algodClient, indexerClient, poolCfg1)
        : await getHumbleswapPoolInfo(Number(poolCfg1.poolId), algodClient, indexerClient, poolCfg1, address);
    }
    let resolvedInfo2 = cachedInfo2;
    if (!resolvedInfo2) {
      resolvedInfo2 = poolCfg2.dex === 'nomadex'
        ? await getNomadexPoolInfo(Number(poolCfg2.poolId), algodClient, indexerClient, poolCfg2)
        : await getHumbleswapPoolInfo(Number(poolCfg2.poolId), algodClient, indexerClient, poolCfg2, address);
    }
    ...
    for (const testCase of testCases) {
      if (testCase.p1Amount === 0n) {
        ...
        const quotes = await Promise.all([
          calculateQuoteForPool(poolCfg1, inputToken, outputToken, testCase.p1Amount.toString(), slippage, address, resolvedInfo1).catch((err) => {
            recordSkip(poolCfg1, err);
            return null;
          }),
          calculateQuoteForPool(poolCfg2, inputToken, outputToken, testCase.p2Amount.toString(), slippage, address, resolvedInfo2).catch((err) => {
            recordSkip(poolCfg2, err);
            return null;
          })
        ]);
      ...
      try {
        const quotes = await Promise.all(
          matchingPools.map((pool, idx) => {
            ...
            return calculateQuoteForPool(pool, inputToken, outputToken, amounts[idx].toString(), slippage, address, cachedInfo).then((q) => {
              recordSuccess(pool);
              return q;
            }).catch((err) => {
              recordSkip(pool, err);
