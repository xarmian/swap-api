...
function getPlatformFeeConfig() {
  ...
  if (trimmed) {
    ...
    if (!/^\d+$/.test(trimmed) || !Number.isInteger(feeBps)) {
      throw new Error(
  ...
  if (numPools === 2) {
    ...
    if (bestSplit) {
      ...
      if (multiPoolOutput > bestSinglePoolOutput) {
        ...
        if (feeBps > 0 && feeAddress) {
          ...
          // every real config (feeBps <= 10000) `uncappedFee <= gain`, making
          // the cap a strict no-op; only a misconfigured feeBps > 10000 hits
          // it, and then capping keeps the fee-adjusted split >= the single-
          // pool baseline so the user is never pushed below it.
          const uncappedFee = (gain * BigInt(feeBps)) / BigInt(10000);
          const feeAmount = uncappedFee > gain ? gain : uncappedFee;
    ...
    if (multiPoolOutput > bestSinglePoolOutput) {
      ...
      if (feeBps > 0 && feeAddress) {
        ...
        // it. No-op for feeBps <= 10000; only a misconfigured feeBps > 10000
        // hits the cap, keeping the fee-adjusted split >= the single-pool
        // baseline so the user is never pushed below it.
        const uncappedFee = (gain * BigInt(feeBps)) / BigInt(10000);
        const feeAmount = uncappedFee > gain ? gain : uncappedFee;
