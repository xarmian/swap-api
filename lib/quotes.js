...
async function findOptimalMultiHopRoute(routes, inputToken, outputToken, amount, slippage, address = '', maxCombinations = 100, poolInfoCache = createPoolInfoCache()) {
  ...
  for (const route of routes) {
    ...
    if (route.pools && Array.isArray(route.pools) && route.pools.length > 0) {
      ...
      // the exact same per-hop -- and therefore final -- output. Re-evaluating
      // it would be pure duplicated work that can only ever tie the already
      // queued poolOptions candidate and, losing the strict-greater tie break,
      // never change the winner. So skip concrete enumeration entirely for
      // single-pool-per-hop routes.
  ...
  // sequential version for the same chain state.
  for (const { candidate, quote, error } of evaluated) {
    ...
    if (type === 'poolOptions') {
      ...
      validRoutes.push({ route: originalRoute, quote, output });
      ...
      // Store the best concrete route for each original route (only if it beats
      // the poolOptions version or poolOptions wasn't evaluated).
      const existingRoute = validRoutes.find(vr => vr.route === originalRoute);
      if (!existingRoute || output > existingRoute.output) {
        const idx = validRoutes.findIndex(vr => vr.route === originalRoute);
        if (idx >= 0) {
          validRoutes[idx] = { route: evalRoute, quote, output };
        } else {
          validRoutes.push({ route: evalRoute, quote, output });
        }
      }
