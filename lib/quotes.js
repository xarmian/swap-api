...
async function findOptimalMultiHopRoute(routes, inputToken, outputToken, amount, slippage, address = '', maxCombinations = MAX_ROUTE_COMBINATIONS, poolInfoCache = createPoolInfoCache()) {
  ...
  // Routes that reserved a concrete slot up front for their FAILURE-ONLY
  // fallback (see the singlePoolPerHop branch below and
  // evaluateConcreteCombinations).
  const reservedFallbackRoutes = new Set();
  ...
  // always kept so every route stays represented by its best split. The budget
  // is claimed strictly in route order - including a reserved slot for each
  // deferred single-pool route's potential fallback (below) - so a LATER
  // route's enumeration can never starve an EARLIER route's candidate. Lowering
  ...
  for (const route of routes) {
    ...
    if (route.pools && Array.isArray(route.pools) && route.pools.length > 0) {
      ...
      if (singlePoolPerHop) {
        ...
        // Reserve ONE concrete slot for this route's potential failure-recovery
        // fallback now, at its position in route order (a single-pool-per-hop
        // route yields exactly one concrete combination). Reserving here - not
        // in the later reduction phase - is what keeps budget allocation
        // strictly route-ordered: a LATER route's up-front enumeration can no
        // longer consume the slot an EARLIER deferred route needs to recover
        // from a transient split failure. The reservation is spent only if the
        // split pass actually fails; on the success path the slot goes unused (a
        // deliberate, bounded trade for order-fairness).
        if (concreteCombinationBudget > 0) {
          concreteCombinationBudget -= 1;
          reservedFallbackRoutes.add(route);
        }
  ...
  // TASK-26: this failure-recovery path is bounded by the RESERVATION the route
  // claimed up front (in route order) - not by re-reading the live budget here,
  // which would let a later route's enumeration retroactively starve this
  // earlier route's recovery. If the route reserved no slot (the budget was
  // already exhausted at its position), it is skipped, so an upstream outage
  // that fails many deferred single-pool routes still can't add concrete
  // retries beyond the per-request cap. A single-pool-per-hop route yields
  // exactly one combination, matching its one reserved slot.
  function evaluateConcreteCombinations(route) {
    if (!reservedFallbackRoutes.delete(route)) return Promise.resolve([]);
    const concreteRoutes = generateRouteCombinations(route, 1);
    return Promise.all(
