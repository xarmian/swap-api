...
async function findOptimalMultiHopRoute(routes, inputToken, outputToken, amount, slippage, address = '', maxCombinations = MAX_ROUTE_COMBINATIONS, poolInfoCache = createPoolInfoCache()) {
  ...
  // TASK-26: this failure-recovery path draws from the SAME shared
  // concreteCombinationBudget as the up-front concrete enumeration above -
  // otherwise, during an upstream outage where many deferred single-pool
  // routes fail their split pass, each could add unbounded concrete retries
  // and defeat the per-request cap. Skip entirely once the budget is spent,
  // and pass/decrement the remaining budget so the cap holds across the whole
  // request (a single-pool-per-hop route yields exactly one combination, so in
  // practice each recovery consumes one slot).
  function evaluateConcreteCombinations(route) {
    if (concreteCombinationBudget <= 0) return Promise.resolve([]);
    const concreteRoutes = generateRouteCombinations(route, concreteCombinationBudget);
    concreteCombinationBudget -= concreteRoutes.length;
    return Promise.all(
