...
async function calculateMultiHopQuote(route, inputToken, outputToken, amount, slippage, address = '') {
  ...
  for (let hopIndex = 0; hopIndex < numHops; hopIndex++) {
    ...
    // - Safety margins in transaction building provide additional buffer
    ...
    if (hasPoolOptions && poolOptions[hopIndex] && poolOptions[hopIndex].length > 0) {
      ...
      console.log(`[calculateMultiHopQuote] Hop ${hopIndex + 1}: Splitting ${currentAmount.toString()} ${currentInputToken} across ${hopPools.length} pools (slippage: ${hopSlippage})`);
