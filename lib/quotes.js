...
import { getTokenDecimals, calculatePriceImpact, calculateOptimalSplitAmount, calculateRate, applySlippageToOutput } from './utils.js';
...
async function calculateQuoteForPool(poolCfg, inputToken, outputToken, amount, slippage, address = '', cachedPoolInfo = null) {
  ...
  if (dex === 'nomadex') {
    ...
    // exactly (no over/underquote). Two sources, no lossy fallback:
    const hasFeeOverride = poolCfg.fee !== undefined && poolCfg.fee !== null;
    let feeScale;
    if (hasFeeOverride) {
      // Operator bp override converts to the on-chain scale losslessly: bps is a
      // multiple of SCALE/10000 = 1e10, so no precision is lost.
      feeScale = BigInt(fee) * (NOMADEX_FEE_SCALE / 10000n);
    } else if (poolInfo.feeScale !== undefined && poolInfo.feeScale !== null) {
      // Live fee: use the RAW uint256 fraction (swap_fee + platform_fee) so the
      // quote reproduces the contract with full sub-basis-point precision. Never
      // reconstruct it from the ceil-rounded display bps, which would underquote.
      feeScale = BigInt(poolInfo.feeScale);
    } else {
