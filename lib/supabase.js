...
export async function logQuoteRequest(data) {
  ...
  try {
    // Fire and forget - don't await, don't throw. Accepted limitation: on a
    // serverless runtime (e.g. Vercel) the function/container can freeze or be
    // torn down immediately after the HTTP response is sent, before this
    // insert's promise settles, silently dropping the audit row. There's no
    // dependency-free way to keep the function alive for it (no @vercel/functions
    // waitUntil() wired into this Express handler), and audit logging is
    // non-critical (it never affects quote/swap correctness), so the occasional
    // lost row is accepted rather than adding a runtime-specific dependency here.
