  ...
  try {
    ...
    if (dex === 'humbleswap') {
      ...
      const poolConfig = {
        ...
        // No hardcoded fee: the live on-chain fee (protoInfo.totFee) is read per
        // quote in quotes.js. A pool config `fee` is treated only as an explicit
        // operator override; discovery must never stamp a guessed default.
      };
      ...
      const poolConfig = {
        ...
        slippageDefault: 0.01
        // No hardcoded fee: the live on-chain fee (swap_fee + factory platform_fee)
        // is read per quote in nomadex.getPoolInfo. A pool config `fee` is treated
        // only as an explicit operator override; discovery must never stamp a guess.
      };
      
      // Store token metadata in cache (use API data if available, otherwise token info)
      // Note: fetchTokenInfo() already caches the full response, so we preserve it when available
      if (tokAId === 0) {
        // Native VOI
        ...
        // Preserve full tokenInfo (includes imageUrl) but merge in preferred values from API pool data
...
// Hard ceiling regardless of what DISCOVERY_CONCURRENCY / options.concurrency
// request - the whole point of a "cap" is that no misconfiguration can
// recreate the unbounded 100-pools-at-once burst this fix removes.
...
function clampConcurrency(value) {
  ...
  return Math.min(value, MAX_DISCOVERY_CONCURRENCY);
}

function resolveConcurrency(explicit) {
  const clampedExplicit = clampConcurrency(explicit);
  if (clampedExplicit !== null) {
    return clampedExplicit;
  }
  const clampedEnv = clampConcurrency(Number(process.env.DISCOVERY_CONCURRENCY));
  if (clampedEnv !== null) {
    return clampedEnv;
  }
 ...
 * worker pool). Returns Promise.allSettled-shaped results in the SAME order as
 * `items`, regardless of completion order, so callers can zip results back up
 * against their inputs.
 *
 * Cold-start discovery previously ran pools one at a time - 100+ sequential
 * Mimir/algod round-trips before the first quote could be served (TASK-19).
 * Firing all of them at once instead would just move the bottleneck to
 * upstream rate limits / connection exhaustion, so this bounds concurrency.
 */
...
/**
 * Discover all pools from a list of pool IDs, in parallel, bounded by a
 * concurrency cap (default 6, override via DISCOVERY_CONCURRENCY or
 * options.concurrency). Failures are collected rather than thrown so a
 * transient blip on one pool never aborts discovery of the rest - the caller
 * (lib/config.js) decides what a partial result means (serve above a success
 * threshold, retry the rest on a TTL).
 * @param {Array<number>} poolIds - Array of pool IDs
 * @param {Algodv2} algodClient - Algod client
 * @param {Indexer} indexerClient - Indexer client
 * @param {{concurrency?: number}} [options]
 * @returns {Promise<{pools: Array<Object>, tokens: Object, failedPools: Array<{poolId: number, error: string}>}>}
 */
export async function discoverAllPools(poolIds, algodClient, indexerClient, options = {}) {
  const concurrency = resolveConcurrency(options.concurrency);
  const pools = [];
  const failedPools = [];

  console.log(`[Discovery] Starting discovery of ${poolIds.length} pool(s) with concurrency ${concurrency}...`);

  const results = await runWithConcurrencyLimit(
    poolIds,
    concurrency,
    (poolId) => discoverPool(poolId, algodClient, indexerClient)
  );
  ...
  results.forEach((result, i) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      console.warn(`[Discovery] Failed to discover pool ${poolId}:`, message);
      failedPools.push({ poolId, error: message });
    }
  });
  ...
  console.log(`[Discovery] Completed: ${pools.length}/${poolIds.length} pools discovered, ${failedPools.length} failed, ${Object.keys(tokens).length} tokens cached`);
...
/**
 * Seed the token-metadata cache from a previously-persisted snapshot (e.g. the
 * on-disk discovery cache in lib/config.js) without making any network calls.
 * Only fills in tokens that aren't already cached, so it can't clobber fresher
 * data from a live discoverPool() call.
 * @param {Object|undefined} tokens - Map of tokenId -> metadata
 */
export function hydrateTokenMetadataCache(tokens) {
  if (!tokens) return;
  for (const [tokenId, metadata] of Object.entries(tokens)) {
    if (!tokenMetadataCache.has(tokenId)) {
      tokenMetadataCache.set(tokenId, metadata);
    }
  }
}
 ...
 * @returns {Array<Object>} Array of token objects with id, symbol, name, decimals, is_wrapped, imageUrl
 */
export function getAllTokens() {
  const tokens = [];
  
  // First pass: Build a map of underlying token IDs to imageUrls from their wrapped versions
  // This allows us to copy imageUrls from wrapped tokens to their underlying (ASA) tokens
  const underlyingToImageUrl = new Map();
  
  for (const [tokenId, fullInfo] of tokenMetadataCache.entries()) {
    // Check if this is a wrapped token with an imageUrl
    const isWrapped = fullInfo.tokenId !== null && 
                     fullInfo.tokenId !== undefined && 
                     fullInfo.tokenId !== '';
    
    if (isWrapped && fullInfo.imageUrl) {
      const underlyingId = String(fullInfo.tokenId);
      // Map underlying token ID to the wrapped token's imageUrl
      underlyingToImageUrl.set(underlyingId, fullInfo.imageUrl);
    }
  }
  
  // Second pass: Build tokens array, using wrapped token imageUrls for underlying tokens when missing
  for (const [tokenId, fullInfo] of tokenMetadataCache.entries()) {
    ...
    // Get imageUrl: prefer the token's own imageUrl, otherwise use the wrapped version's imageUrl if available
    let imageUrl = fullInfo.imageUrl || null;
    if (!imageUrl && !isWrapped) {
      // This is an unwrapped token without an imageUrl, check if there's a wrapped version with one
      const wrappedImageUrl = underlyingToImageUrl.get(String(tokenId));
      if (wrappedImageUrl) {
