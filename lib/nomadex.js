...
// Nomadex fee scale, verified on-chain against the pool's swap TEAL: output is
// computed as amountOut = amountIn * reserveOut * (SCALE - swap_fee - platform_fee)
// / ((amountIn + reserveIn) * SCALE), with SCALE == 1e14 (both the numerator
// constant and the denominator constant decode to 100000000000000). Both
// swap_fee (pool state) and platform_fee (factory state) are uint256 fractions of
// this SCALE. Converting to basis points (out of 10000) divides by SCALE/10000.
export const NOMADEX_FEE_SCALE = 100000000000000n; // 1e14

/**
 * Parse an application's global state (algosdk 3.x `globalState`, or the raw REST
 * `global-state` shape) into a { key: decodedValue } map. Shared by the pool and
 * factory reads so both use the single decoding path introduced by TASK-13.
 * @param {Object} params - application.params from an indexer lookup
 * @returns {Object<string, Buffer|BigInt|null>}
 */
function parseGlobalState(params) {
...
/**
 * Decode a Nomadex uint256 fee stored as a big-endian byteslice into a BigInt.
 * Nomadex stores swap_fee (pool global state) and platform_fee (factory global
 * state) as 32-byte big-endian byteslices, NOT uint64 fields, so decodeStateValue
 * yields a Buffer for them. Returns null when the value is missing or not a
 * byteslice so callers can throw rather than silently defaulting a fee to zero.
 * @param {Buffer|BigInt|null} value - decoded global-state value
 * @returns {BigInt|null}
 */
function decodeUint256Bytes(value) {
...
/**
 * Read the Nomadex factory's platform_fee (a uint256 byteslice) from the factory
 * app referenced by the pool. The on-chain swap deducts swap_fee + platform_fee,
 * so an honest quote must include both. Throws if the factory app or its
 * platform_fee cannot be read — it never silently drops the platform fee, which
 * would under-apply the fee and overstate output.
 * @param {BigInt|number} factoryId - factory application id
 * @param {Indexer} indexerClient - Indexer client
 * @returns {Promise<BigInt>} platform fee as a uint256 fraction of NOMADEX_FEE_SCALE
 */
async function readPlatformFee(factoryId, indexerClient) {
  // Pass the app id through as-is (algosdk 3.x accepts BigInt); coercing to
  // Number would corrupt ids above Number.MAX_SAFE_INTEGER and read the wrong app.
  const factoryInfo = await indexerClient.lookupApplications(factoryId).do();
  const params = factoryInfo?.application?.params;
  if (!params) {
    throw new Error(`Nomadex factory ${factoryId} application info unavailable`);
  }
  const platformFee = decodeUint256Bytes(parseGlobalState(params)['platform_fee']);
  if (platformFee === null) {
    throw new Error(`Nomadex factory ${factoryId} has no readable platform_fee in global state`);
  }
  return platformFee;
}
...
export async function getPoolInfo(poolId, algodClient, indexerClient, poolConfig = null) {
  try {
    ...
    // Parse global state into a { key: decodedValue } map (algosdk 3.x shape,
    // with the raw REST shape also accepted) via the shared decoder.
    const stateMap = parseGlobalState(appInfo.application.params);


    // Read the live on-chain fee. Nomadex stores the pool fee as a uint256
    // byteslice under `swap_fee`, and the factory (referenced by the pool's
    // `factory` id) adds a `platform_fee`, also a uint256 byteslice. The on-chain
    // swap deducts BOTH as a fraction of NOMADEX_FEE_SCALE, so an honest quote
    // must include both. We convert the combined fee to basis points, rounding UP
    // (ceil) so a quote can never under-apply the fee and overstate output. There
    // is NO silent default: if swap_fee, the factory id, or platform_fee is
    // unreadable we throw so the caller skips this pool rather than quoting
    // against a guessed fee.
    const swapFee = decodeUint256Bytes(stateMap['swap_fee']);
    if (swapFee === null) {
      throw new Error(`Nomadex pool ${poolId} has no readable swap_fee in global state`);
    }
    const factoryId = stateMap['factory'];
    if (typeof factoryId !== 'bigint') {
      throw new Error(`Nomadex pool ${poolId} has no readable factory id in global state`);
    }
    const platformFee = await readPlatformFee(factoryId, indexerClient);

    const totalFeeRaw = swapFee + platformFee; // uint256 fraction of NOMADEX_FEE_SCALE
    if (totalFeeRaw < 0n || totalFeeRaw >= NOMADEX_FEE_SCALE) {
      throw new Error(`Nomadex pool ${poolId} total fee ${totalFeeRaw} out of range for scale ${NOMADEX_FEE_SCALE}`);
    }
    // feeBps = ceil(totalFeeRaw * 10000 / NOMADEX_FEE_SCALE)
    const fee = (totalFeeRaw * 10000n + (NOMADEX_FEE_SCALE - 1n)) / NOMADEX_FEE_SCALE;
    ...
    return {
      ...
      // `fee` is the display/back-compat basis-point view (ceil, never
      // under-applied). `feeScale` is the RAW on-chain fraction of
      // NOMADEX_FEE_SCALE (swap_fee + platform_fee), which calculateOutputAmount
      // uses directly so the quote matches the contract with full sub-bp
      // precision — no rounding loss from the bp conversion.
      fee: Number(fee),
      feeScale: totalFeeRaw.toString(),
      tokA: tokA,
      tokB: tokB
    };
  } catch (error) {
...
/**
 * Calculate output amount, matching the Nomadex pool's on-chain swap subroutine
 * BIT-FOR-BIT (pool approval program, `label109`; verified by disassembling app
 * 411756). The contract computes, with SCALE = NOMADEX_FEE_SCALE = 1e14
 * (0x5af3107a4000) and totalFee = swap_fee + platform_fee (uint256 fractions of
 * SCALE):
 *
 *   out = amountIn * reserveOut * (SCALE - totalFee) / ((amountIn + reserveIn) * SCALE)
 *
 * i.e. the fee is applied ONLY to the numerator, and the denominator uses the
 * RAW (amountIn + reserveIn) — NOT a Uniswap-style fee-adjusted input. A single
 * trailing byteslice division (`b/`) truncates toward zero.
 *
 * The fee is passed in the SAME units the contract uses: `feeScale`, a fraction
 * of NOMADEX_FEE_SCALE (i.e. the raw `swap_fee + platform_fee`). This preserves
 * full sub-basis-point precision so the quote equals the contract output exactly
 * — passing a basis-point value would discard precision below one bp (see the
 * caller in lib/quotes.js, which converts an operator bp override to this scale
 * exactly via feeBps * (SCALE / 10000), and otherwise uses the raw on-chain
 * fraction from getPoolInfo's `feeScale`).
 *
 * All math is BigInt (no 2^53 overflow), with one trailing floor division to
 * mirror the contract's single `b/` truncation. Callers guarantee reserves > 0
 * (getPoolInfo rejects non-positive reserves), so the denominator is > 0.
 *
 ...
 * @param {BigInt|string|number} feeScale - Total fee as a fraction of
 *   NOMADEX_FEE_SCALE (1e14); e.g. 2e12 == 200 bps. MUST be in [0, SCALE).
 * @returns {BigInt} Output amount
 */
export function calculateOutputAmount(inputAmount, inputReserve, outputReserve, feeScale) {
  const amountIn = BigInt(inputAmount);
  const reserveIn = BigInt(inputReserve);
  const reserveOut = BigInt(outputReserve);
  const fee = BigInt(feeScale); // fraction of NOMADEX_FEE_SCALE, matches the TEAL

  // On-chain amounts and reserves are unsigned. Reject non-positive inputs
  // rather than dividing by a zero/negative denominator (amountIn === -reserveIn
  // would divide by zero; other negatives yield a bogus positive output). This
  // also guarantees denominator = (amountIn + reserveIn) * SCALE > 0.
  if (amountIn <= 0n || reserveIn <= 0n || reserveOut <= 0n) {
    return 0n;
  }

  // Fee on the numerator only; RAW (amountIn + reserveIn) * SCALE denominator.
  const numerator = amountIn * reserveOut * (NOMADEX_FEE_SCALE - fee);
  const denominator = (amountIn + reserveIn) * NOMADEX_FEE_SCALE;
