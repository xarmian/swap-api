...
// --- Per-request route-combination fan-out cap (TASK-26) ---
//
// generateRouteCombinations/findOptimalMultiHopRoute evaluate up to
// maxCombinations concrete pool combinations per candidate multi-hop route,
// each combination re-fetching live pool state - the main amplification
// lever behind a single /quote fanning out into many upstream calls. On
// Vercel there's no platform-level throttle, so this cap (together with the
// best-effort per-IP rate limiter in index.js) is the primary DoS defense.
// Lowering it changes which candidate split wins, so a user can get a very
// slightly worse quote (CONVE-35) - 10 is the audit's deliberate default
// (diminishing returns evaluating more). Env-tunable so it can be raised
// without a deploy if a real route legitimately needs more candidates.
// Upper bound on the accepted value: a positive integer, but capped at
// MAX_ROUTE_COMBINATIONS_CEILING so a pathological env value (e.g. "1e100",
// which Number.isInteger accepts) can't silently restore effectively-unbounded
// enumeration and defeat the whole abuse control. 1000 is 10x the pre-audit
// default of 100 - generous headroom for legitimately raising the cap without
// a deploy, while still bounded. Anything outside [1, 1000] (unset,
// non-numeric, 0, negative, fractional, or absurdly large) falls back to 10.
const MAX_ROUTE_COMBINATIONS_CEILING = 1000;
const MAX_ROUTE_COMBINATIONS = (() => {
  const fromEnv = Number(process.env.MAX_ROUTE_COMBINATIONS);
  return Number.isInteger(fromEnv) && fromEnv >= 1 && fromEnv <= MAX_ROUTE_COMBINATIONS_CEILING
    ? fromEnv
    : 10;
})();
...
// Pure projection of the failed-pool bookkeeping into the wire shape, with the
// raw `error` included ONLY when includeErrors is true. Extracted and exported
// so the redaction contract (TASK-60) is unit-testable directly, without
// standing up live discovery state. `entries` is Array.from(map.entries()).
function projectFailedPools(entries, includeErrors) {
  return entries.map(([poolId, info]) => ({
    poolId,
    attempts: info.attempts,
    lastAttempt: new Date(info.lastAttempt).toISOString(),
    ...(includeErrors ? { error: info.error } : {})
  }));
}
 ...
 *
 * The per-pool `error` is the RAW discovery-failure message off algod/indexer/
 * ulujs (lib/discovery.js) and can carry node-provider URLs / HTTP internals,
 * so it is only included when `includeErrors` is true — an operator-gated
 * caller (GET /health, /config/pools; see isOperatorRequest in index.js). Every
 * public caller omits it and gets the operator-visibility signal (which pools
 * failed, attempt counts, degraded state) without the raw text (TASK-60/-19).
 * The default is false, so no caller leaks the raw error unless it opts in.
 */
function getDiscoveryStatus(includeErrors = false) {
  return {
    ...
    failedPools: projectFailedPools(Array.from(failedPoolIds.entries()), includeErrors)
  };
}
...
/**
 * Resolve a pool config by its numeric poolId, optionally constrained to a DEX
 * filter. Algorand app ids are globally unique, so a poolId identifies exactly
 * one pool today; honoring `dexFilter` here keys the explicit-pool lookup on the
 * composite (dex, poolId) -- matching the pool cache-key discipline (PR #23,
 * `${dex||'humbleswap'}:${poolId}`) -- so a caller that supplies both a poolId
 * and a `dex` filter can never be handed a pool from a DEX they excluded.
 * @param {string|number} poolId - numeric pool id
 * @param {Array<string>|undefined} [dexFilter] - optional list of allowed DEXes
 *   (same shape as findMatchingPools). Omitted/empty => any DEX (unchanged
 *   behavior for callers that don't pass a filter).
 * @returns {Object|null}
 */
function getPoolConfigById(poolId, dexFilter) {
  loadConfigsOnce();
  ...
  const allowedDexes = dexFilter && Array.isArray(dexFilter) && dexFilter.length > 0
    ...
    : null;
  const found = poolsConfig.pools.find(p => {
    if (Number(p.poolId) !== pid) return false;
    if (allowedDexes && !allowedDexes.includes((p.dex || 'humbleswap').toLowerCase())) return false;
    return true;
  });
  return found || null;
}
    ...
    if (currentToken === outputTokenNum) {
      if (path.length > 0) {
        ...
        for (let i = 0; i < path.length; i++) {
          ...
          // Check if pool already added (avoid duplicates). CONVE-32: key on
          // (dex, poolId), matching the pool-info cache key discipline, so DEX
          // identity is never dropped from the de-dup (Algorand app ids are
          // globally unique today, so this is behavior-preserving now, but it
          // keeps the key honest if that ever changes).
          if (!route.poolOptions[i].some(p => p.poolId === poolCfg.poolId && (p.dex || 'humbleswap') === (poolCfg.dex || 'humbleswap'))) {
      ...
      if (neighbors) {
        for (const neighbor of neighbors) {
          ...
          if (neighbor.otherToken === hopOutputToken) {
            ...
            // Check if pool already added (avoid duplicates). CONVE-32: key on
            // (dex, poolId) to match the pool-info cache key discipline (see
            // the sibling de-dup above).
            if (!route.poolOptions[hopIndex].some(p => p.poolId === poolCfg.poolId && (p.dex || 'humbleswap') === (poolCfg.dex || 'humbleswap'))) {
 ...
 * @param {number} maxCombinations - Maximum number of combinations to generate (default: MAX_ROUTE_COMBINATIONS, env-tunable, TASK-26)
 * @returns {Array<Object>} Array of concrete routes: [{pools: [pool1, pool3], intermediateTokens: [...], hops: 2}, ...]
 */
function generateRouteCombinations(routeWithOptions, maxCombinations = MAX_ROUTE_COMBINATIONS) {
  ...
  if (totalCombinations > maxCombinations) {
    ...
    // TASK-26: the per-hop maxPoolsPerHop = ceil(maxCombinations^(1/hops))
    // rounding can OVERSHOOT maxCombinations (e.g. ceil(sqrt(10)) = 4 -> 4*4 =
    // 16 for a 2-hop route), so hard-truncate to the exact cap. Combinations
    // are produced in a stable, deterministic order by
    // generateCombinationsRecursive, so slicing keeps the first maxCombinations
    // candidates reproducibly - never more than the cap promises.
