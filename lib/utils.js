...
async function getTokenDecimals(tokenId) {
  ...
  try {
    ...
    if (resp && resp.asset && typeof resp.asset.params.decimals === 'number') {
      const dec = resp.asset.params.decimals;
      decimalsCache.set(key, dec);
      return dec;
    }
    // The indexer responded but the asset/decimals field was missing or
    // malformed (shouldn't normally happen). Treat it exactly like a failed
    // lookup: do NOT cache the fallback, so a later well-formed response can
    // still populate the cache correctly instead of being stuck at 6 forever.
    console.warn(`getTokenDecimals: indexer response for token ${key} missing decimals, using fallback decimals=6 for this request only (not cached)`);
    return 6;
  } catch (e) {
    // Do NOT cache on a failed lookup: caching here would permanently poison
    // the decimals cache with the fallback (6) after a single transient
    // indexer failure, silently mispricing every future quote for this token
    // (CONVE-35). Return the fallback for THIS request only so the next
    // request retries the indexer lookup and can self-heal.
    console.warn(`getTokenDecimals: indexer lookup failed for token ${key}, using fallback decimals=6 for this request only (not cached):`, e.message);
    return 6;
  }
}
